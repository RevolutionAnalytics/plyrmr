...
# Features
* redesigned the API to avoid duplication, split jobs between the all powerful `transmute`, the `transform`-like but better-named `bind.cols` and the specialized, `dplyr`-lifted `select`.
* `magic.wand` now knows special tricks to deal with non-standard args

...
Any good API provides basic elements, ways to modify them and ways to combine them. This way a wide variety of computations can be described in a language concise enough for people to understand. plrymr is no different in that all the above functions can be freely combined. In the following example, we see how to create a new column and then apply a filter based on that column.
...
The bonus feature is that each call, run in isolation, requires a map-reduce job. Combined, they still require one. This is the kinds of optimizations allowed by delayed evaluation.

Deeply nested expressions can become hard to read since the name of the functions can be arbitrarily far from all but one of its arguments. Some sort of "syntax locality" principle is violated. To address that we can adopt two alternate stiles. The first involved assigning intermediate results to temporary variables, as in
...
It will do exactly what you expect for simple functions that can be applied to each chunk of data. For instance, selecting a certain column fits this profile, as does filtering rows based on the content of each row independently. But if, for instance, you wanted to extract the first few rows of a data set with `head`, and combined that with `gapply`, you would end up with a fairly arbitrary selection of rows. That's where the art of mapreduce programming kicks in.
